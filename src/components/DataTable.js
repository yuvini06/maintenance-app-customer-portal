...
const getColumns = () => (["Task ID", "Planned Start Time", "Planned End Time", "Status"]);
...
const generateData = (row, props) => {
  ...
  for (const [key, value] of Object.entries(row)) {
    if (key === "risk") {
      ...
      } else if (row.status === STATUS_TYPES.IN_PROGRESS && key === "actual_end_time") {
        cells.push(renderCell(" —"));
      } else {
        cells.push(renderCell(getFormattedTimestamp(value)));
      }
      ...
      cells.push(
        <Button
          ...
          size="small"
          sx={{ marginTop: "1rem" }}
          onClick={() => props.toggleUpdateModal(true, { 
            taskId: row.task_id, 
